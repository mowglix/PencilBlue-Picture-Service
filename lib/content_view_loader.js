...
module.exports = function(pb) {
    ...
    ContentViewLoader.prototype.renderGalleryElement = function(mediaId, options, cb) {
        ...
        self.mediaService.loadById(mediaId, function(err, descriptor) {
            var thumbPath = "/images/"  + descriptor.location.substring(7);
            var gts = self.ts.getChildInstance();
            if(descriptor.media_type === 'image') {
                gts.registerLocal('article_gallery_elem_url_thumb', thumbPath);
                gts.registerLocal('article_gallery_elem_url', descriptor.location);
                gts.registerLocal('article_gallery_elem_name', descriptor.name);
